...
import { ArrowLeft, BarChart3, Clock, Brain, Target, TrendingUp, Sparkles, Activity } from "lucide-react";
...
export default function AnalyticsPage() {
  ...
  // Learning DNA data for Radar Chart
  const dnaData = [
    { subject: 'Reading Speed', A: (profile.reading_speed_wpm / 300) * 100, fullMark: 100 },
    { subject: 'Accuracy', A: profile.avg_quiz_accuracy || 0, fullMark: 100 },
    { subject: 'Persistence', A: (profile.persistence_score || 0.5) * 100, fullMark: 100 },
    { subject: 'Autonomy', A: (1 - (profile.hint_usage_rate || 0)) * 100, fullMark: 100 },
    { subject: 'Completion', A: (profile.completion_rate || 0) * 100, fullMark: 100 },
  ];
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8" data-testid="analytics-summary">
              ...
              <div className="neura-card p-5 bg-[#C8B6FF]/10">
                <TrendingUp className="w-6 h-6 text-[#C8B6FF] mb-2" />
                <p className="text-2xl font-bold text-[#0F172A]" style={{ fontFamily: 'Fredoka, sans-serif' }}>{avgSessionScore}%</p>
                <p className="text-sm font-bold text-[#64748B]">Avg Score</p>
              </div>
            </div>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              {/* Learning DNA Radar */}
              <div className="neura-card p-6 bg-gradient-to-br from-[#118AB2]/5 to-white" data-testid="chart-dna">
                <div className="flex items-center gap-2 mb-4">
                  <Activity className="w-5 h-5 text-[#118AB2]" />
                  <h3 className="font-bold text-[#0F172A]" style={{ fontFamily: 'Fredoka, sans-serif' }}>Learning DNA (Adaptive Profile)</h3>
                </div>
                <ResponsiveContainer width="100%" height={300}>
                  <RadarChart cx="50%" cy="50%" outerRadius="80%" data={dnaData}>
                    <PolarGrid stroke="#e2e8f0" />
                    <PolarAngleAxis dataKey="subject" tick={{ fontSize: 10, fontWeight: 700, fill: '#334155' }} />
                    <PolarRadiusAxis angle={30} domain={[0, 100]} tick={false} axisLine={false} />
                    <Radar name="Student" dataKey="A" stroke="#118AB2" fill="#118AB2" fillOpacity={0.6} />
                  </RadarChart>
                </ResponsiveContainer>
