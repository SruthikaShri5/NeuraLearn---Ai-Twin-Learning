...
import { Brain, Mail, Lock, User, Eye, EyeOff, ArrowRight, ArrowLeft, Check,
  BookOpen, GraduationCap, Users, School, Layers, ChevronDown } from "lucide-react";

// ── Native select wrapper — no Radix, no ResizeObserver errors ──────────────
function NativeSelect({ value, onChange, options, id, testId, placeholder }) {
  return (
...
const LEARNING_STYLES = [
  { value: "visual", label: "Visual (seeing)" }, { value: "audio", label: "Auditory (hearing)" },
  { value: "reading", label: "Reading/Writing" }, { value: "interactive", label: "Interactive (doing)" },
];
...
export default function SignupPage() {
  ...
  if (step === 0) {
    return (
      <div className="h-screen overflow-y-auto flex items-center justify-center px-6 py-6" style={{ background: "#FAFAFA", color: "#1A1A2E" }} data-testid="signup-page">
        <main id="main-content" className="w-full max-w-lg">
          <Link to="/" className="flex flex-col items-center gap-0.5 mb-4 justify-center">
          ...
          <div className="neura-card p-5">
            <h1 className="text-xl font-bold text-[#0F172A] text-center mb-1" style={{ fontFamily: 'Fredoka, sans-serif' }}>Who are you?</h1>
            <p className="text-[#64748B] text-center text-sm mb-4">Choose your role to get started</p>
            <div className="space-y-3">
              {Object.entries(ROLE_CONFIG).map(([id, cfg]) => (
                <button key={id} onClick={() => { setRole(id); setStep(1); }}
                  ...
                  <div className={`w-10 h-10 rounded-xl ${cfg.color} flex items-center justify-center border-2 border-[#0F172A] shrink-0`}>
                    <cfg.icon className="w-5 h-5 text-white" />
                  </div>
                  <div className="text-left">
                    <p className="font-bold text-[#0F172A]" style={{ fontFamily: 'Fredoka, sans-serif' }}>{cfg.label}</p>
                    <p className="text-xs text-[#64748B]">{cfg.desc}</p>
                  </div>
                  <ArrowRight className="w-4 h-4 text-[#64748B] ml-auto" />
                </button>
              ))}
            </div>
          ...
          {step === 1 && (
            <form className="space-y-4 mt-4" onSubmit={(e) => {
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <Input id="name" value={form.name} onChange={(e) => updateForm("name", e.target.value)}
                    ...
                    className="pl-10 h-12 border-2 border-[#0F172A] rounded-xl" data-testid="signup-name-input" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <Input id="email" type="email" value={form.email} onChange={(e) => updateForm("email", e.target.value)}
                    ...
                    className="pl-10 h-12 border-2 border-[#0F172A] rounded-xl" data-testid="signup-email-input" />
                </div>
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setStep(0)} className="neura-btn bg-white text-[#0F172A] flex-1 h-12" data-testid="signup-back-role">
                  <ArrowLeft className="w-4 h-4" /> Back
                </button>
                <button type="submit" className="neura-btn bg-[#118AB2] text-white flex-1 h-12" data-testid="signup-next-step-1">
            ...
            <div className="space-y-4 mt-4">
              <div>
                ...
                <NativeSelect
                  ...
                  onChange={(v) => updateForm("grade_level", v)}
                  options={GRADES}
                  testId="signup-grade-select"
                />
              </div>
              <div>
                ...
                <NativeSelect
                  ...
                  onChange={(v) => updateForm("disability_type", v)}
                  options={DISABILITY_TYPES}
                  testId="signup-disability-select"
                />
              </div>
            ...
            <div className="space-y-3 mt-3">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={handleSubmit} disabled={loading} className="neura-btn bg-[#06D6A0] text-[#0F172A] flex-1 h-12 disabled:opacity-50" data-testid="signup-submit-btn">
                  {loading ? "Creating your twin..." : <><Check className="w-4 h-4" /> Create Account</>}
                </button>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4 mt-4">
              ...
              <div>
                ...
                <NativeSelect
                  ...
                  onChange={(v) => updateForm("subject_specialization", v)}
                  options={SUBJECTS}
                  testId="signup-subject-select"
                />
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setStep(1)} className="neura-btn bg-white text-[#0F172A] flex-1 h-12">
