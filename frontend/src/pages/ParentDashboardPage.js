...
import ParentConnectionPanel from "@/components/ParentConnectionPanel";
import LinkChildPanel from "@/components/LinkChildPanel";
...
export default function ParentDashboardPage() {
  ...
  const [ptmConnections, setPtmConnections] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [childrenRes, lessonsRes, ptmRes] = await Promise.all([
          api.get("/parent/children-progress"),
          api.get("/parent/grade-lessons"),
          api.get("/ptm/connections").catch(() => ({ data: { connections: [] } })),
        ]);
        const kids = childrenRes.data.children || [];
        setChildren(kids);
        setLessons(lessonsRes.data.lessons || []);
        setPtmConnections(ptmRes.data.connections || []);
  ...
  // ── Tip of the Day (rule-based, no AI needed) ──────────────────────────
  const getTipOfDay = (child) => {
    if (!child) return null;
    const score = child.avg_score || 0;
    const streak = child.streak || 0;
    const concepts = child.concepts_mastered || 0;
    const time = child.total_time_minutes || 0;

    const tips = [];

    if (streak === 0) {
      tips.push({ icon: "🔥", title: "Build a streak!", text: `${child.name?.split(" ")[0]} hasn't studied today. Even 10 minutes builds momentum — encourage a quick session!` });
    } else if (streak >= 7) {
      tips.push({ icon: "🏆", title: "Amazing streak!", text: `${child.name?.split(" ")[0]} has studied ${streak} days in a row! Celebrate this consistency — it's the key to long-term learning.` });
    }

    if (score < 50 && concepts > 0) {
      tips.push({ icon: "💡", title: "Review time", text: `Quiz scores are below 50%. Try reviewing lessons together before attempting quizzes — understanding beats memorising.` });
    } else if (score >= 80) {
      tips.push({ icon: "⭐", title: "High performer!", text: `${child.name?.split(" ")[0]} is scoring above 80%! Consider enabling harder difficulty or exploring the next grade's topics.` });
    }

    if (time < 15) {
      tips.push({ icon: "⏱️", title: "Short sessions work!", text: "Research shows 15–20 minute focused sessions are more effective than long cramming. Encourage short daily practice." });
    }

    if (concepts >= 5) {
      tips.push({ icon: "🗺️", title: "Check the Knowledge Graph", text: `${child.name?.split(" ")[0]} has mastered ${concepts} concepts! Open the Knowledge Graph to see which topics are next.` });
    }

    // Default tip
    if (tips.length === 0) {
      tips.push({ icon: "💬", title: "Ask about today's lesson", text: "Ask your child to explain what they learned today in their own words. Teaching others is the best way to solidify understanding." });
    }
  ...
  const tip = getTipOfDay(selectedChild);
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA]" data-testid="parent-dashboard">
      ...
      <main id="main-content" className="max-w-7xl mx-auto px-6 py-8">
        {/* Welcome */}
        <div className="neura-card p-6 mb-6 bg-gradient-to-r from-[#FFD166]/20 to-[#FFD6BA]/20">
        ...
        {/* Tip of the Day */}
        {tip && (
          <div className="neura-card p-5 mb-6 flex items-start gap-4 bg-[#FFFBEB] border-[#FFD166]">
            <span className="text-3xl shrink-0">{tip.icon}</span>
            <div>
              <p className="font-bold text-[#b8860b] text-sm mb-0.5">💡 Tip of the Day</p>
              <p className="font-bold text-[#0F172A]" style={{ fontFamily: 'Fredoka, sans-serif' }}>{tip.title}</p>
              <p className="text-sm text-[#374151] mt-1 leading-relaxed">{tip.text}</p>
            </div>
          </div>
        )}

        {/* Parent-Teacher Connection Panel */}
        <ParentConnectionPanel
          connection={ptmConnections[0] || null}
          studentId={selectedChild?.id || null}
          onConnect={(conn) => setPtmConnections([conn, ...ptmConnections])}
          onMessage={(id, name) => { setChatUserId(id); setChatUserName(name); setShowChat(true); }}
        />

        {/* Link Child Panel */}
        <LinkChildPanel onLinked={(child) => setChildren([child, ...children])} />
