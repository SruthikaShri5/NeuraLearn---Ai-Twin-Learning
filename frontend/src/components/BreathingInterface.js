...
const BREATHING_ORB = "/mascot.svg";
...
export default function BreathingInterface() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center" style={{ background: "linear-gradient(135deg, #EEF2FF 0%, #F0FDF4 100%)" }} data-testid="breathing-interface" role="dialog" aria-label="Breathing exercise">
      <button onClick={close} className="absolute top-6 right-6 text-[#6B7280] hover:text-[#1A1A2E] p-2 rounded-xl border border-[#E5E7EB] hover:bg-[#F3F4F6]" data-testid="breathing-close-btn" aria-label="Close breathing exercise">
        <X className="w-8 h-8" />
      </button>
      ...
      <div className="text-center">
        ...
        <div aria-live="assertive">
          <h2 className="text-4xl font-bold text-[#1A1A2E] mb-2" style={{ fontFamily: 'Fredoka, sans-serif' }} data-testid="breathing-phase">
            {phase.name}
          </h2>
          <p className="text-6xl font-bold text-[#118AB2]" style={{ fontFamily: 'Fredoka, sans-serif' }} data-testid="breathing-timer">
            {timer}
          </p>
        </div>
        ...
        <p className="text-[#6B7280] text-sm mt-6">Press <kbd className="px-2 py-1 bg-[#F3F4F6] rounded text-[#374151] border border-[#E5E7EB]">Esc</kbd> or click X to close</p>
      </div>
    </div>
  );
}
