...
export default function AccessibilityBot() {
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className={`fixed bottom-4 right-4 w-12 h-12 rounded-full border-2 border-[#0F172A] shadow-[3px_3px_0px_#0F172A] flex items-center justify-center z-50 transition-all ${
      ...
      {/* Panel */}
      {open && (
        <div className="fixed bottom-20 right-4 w-72 neura-card p-4 z-50 bg-white" role="dialog" aria-label="Accessibility Bot" onClick={(e) => e.stopPropagation()}>
